...
type InstructorFormInnerProps = {
  onCreateInstructor: (value: InstructorFormSchema) => void;
  isPending: boolean;
};

export const InstructorFormInner: React.FC<InstructorFormInnerProps> = ({
  onCreateInstructor,
  isPending,
}) => {
  ...
  return (
    <form onSubmit={form.handleSubmit(onCreateInstructor)}>
      <div className="space-y-6">
        ...
        <div className="w-full">
          <Button type="submit" className="w-full" disabled={isPending}>
            {isPending ? "Loading..." : "Register Instructor!"}
          </Button>
        </div>
      </div>
    </form>
